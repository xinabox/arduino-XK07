...
bool xProvision::receive(void)
{
    ...
    else
    {
        ...
        if (strncmp(syncData.c_str(), "$!$$!$START", 11) == 0)
        {
            ...
            while (!Serial.available())
            {
                ui.loading("     Provisioning");
            };
            ui.provPass(" PROV PASS");
...
uint8_t xProvision::loadFile(String &json)
{
    ...
    //delay(3000);
    // Serial.println();
    // Serial.println("loadfile() : " + String(millis()));
    if (SPIFFS.begin())
    {
       // Serial.println("SPIFFS.begin() : " + String(millis()));
...
void xProvision::radioSelect()
{
    StaticJsonDocument<256> doc;
    JsonArray data = doc.createNestedArray("Radio");
    data.add("RL01");
    data.add("RL02");
    data.add("RL03");
